...
class ACTOptimizer:
    ...
    def extract_target_v_summary_features(self, target_V):
        ...
        if coefs_loaded:
            ...
            if self.segregation_arima_order:
                arima_order = self.segregation_arima_order
        ...
        if self.train_amplitude_frequency:
            target_amplitude, target_frequency = utils.get_amplitude_frequency(
                target_V.float(), self.inj_dur, self.inj_start, fs=self.fs
            )
        ...
        if self.train_mean_potential:
            target_mean_potential = utils.get_mean_potential(
                target_V.float(), self.inj_dur, self.inj_start
            )
            if target_summary_features is not None:
                target_summary_features = torch.cat(
                    (
