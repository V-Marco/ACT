...
class DefaultSummaryModel(torch.nn.Module):
    ...
    def __init__(self, num_summary_features: int = 8, use_statistics: bool = True) -> None:
        '''
        ...
        self.linear = torch.nn.Linear(in_features = 64, out_features = num_summary_features)

        print(f"Total number of summary features: {num_summary_features + use_statistics * 3}")

    def forward(self, X):

        X = X.reshape((-1, 1, 1024)) # (batch_size, 1, 1024)
