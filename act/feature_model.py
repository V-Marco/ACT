...
class DefaultSummaryModel(torch.nn.Module):
    ...
    def __init__(self, num_linear_layer_input_features: int, num_summary_features: int = 8, use_statistics: bool = True) -> None:
        '''
        ...
        self.conv1 = torch.nn.Conv1d(in_channels = 1, out_channels = 1, kernel_size = 9, padding = "same") # 1024 -> 1024
        self.pool1 = torch.nn.MaxPool1d(kernel_size = 2, stride = 2) # 1024 -> 512
        self.conv2 = torch.nn.Conv1d(in_channels = 1, out_channels = 1, kernel_size = 7, padding = "same") # 512 -> 512
        self.pool2 = torch.nn.MaxPool1d(kernel_size = 2, stride = 2) # 512 -> 256
        self.conv3 = torch.nn.Conv1d(in_channels = 1, out_channels = 1, kernel_size = 5, padding = "same") # 256 -> 256
        self.pool3 = torch.nn.MaxPool1d(kernel_size = 2, stride = 2) # 256 -> 128
        self.conv4 = torch.nn.Conv1d(in_channels = 1, out_channels = 1, kernel_size = 3, padding = "same") # 128 -> 128
        self.pool4 = torch.nn.MaxPool1d(kernel_size = 2, stride = 2) # 128 -> 64
        self.linear = torch.nn.Linear(in_features = num_linear_layer_input_features, out_features = num_summary_features)
        
        print(f"Total number of summary features: {num_summary_features + use_statistics * 3}")
